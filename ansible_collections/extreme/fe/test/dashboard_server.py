    ...
    (function () {
        ...
        function renderConfigOptions() {
            ...
            if (!configEntries.length) {
                ...
                empty.textContent = 'No test files found in test/components.';
...
SUMMARY_ROOT = BASE_DIR / "test"
SUMMARY_PATTERN = "test*.yml"
DEFAULT_SUMMARY = "test.yml"
CONFIG_ROOT = BASE_DIR / "test" / "components"
CONFIG_PATTERN = "test_*.yml"
DEFAULT_SUMMARY_PATH = SUMMARY_ROOT / DEFAULT_SUMMARY
INCLUDE_PREFIX = "- include "
INVENTORY_ROOT = BASE_DIR / "test" / "cfg"
SSH_HELPER_SCRIPT = BASE_DIR / "test" / "tools" / "ssh_fe_dt.exp"
    ...
    if inventory is not None:
        ...
        if trimmed:
            ...
            if trimmed in available_inventory:
                inventory_entry_value = f"test/cfg/{trimmed}"
            else:
...
class RunManager:
    ...
    async def start(self, summary_name: str) -> Path:
        ...
        async with self._lock:
            ...
            cmd = [
                sys.executable,
                str((BASE_DIR / "test" / "run_test.py").resolve()),
                str(summary_path),
            ]
