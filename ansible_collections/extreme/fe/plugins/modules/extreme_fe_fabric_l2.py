...
options:
  state:
        description:
            ...
            - "C(replaced) treats the supplied values as authoritative for the targeted ISID."
            - "C(overridden) enforces the supplied values and clears the friendly name when it is omitted."
            - "C(deleted) removes the ISID binding from the device."
  ...
  isid:
    description:
      ...
      - Required when C(state) is not C(gathered).
    type: int
  isid_type:
    description:
  ...
  name:
    description:
      ...
      - When C(state) is C(overridden) and C(name) is omitted, the module clears the existing friendly name.
    type: str
  cvlan:
    description:
