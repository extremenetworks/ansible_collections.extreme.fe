...
class MlagModule:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.module.fail_json(
                msg=f"Unexpected error: {to_text(e)}\nTraceback: {traceback.format_exc()}"
            )
    ...
    def _handle_present(self) -> Dict[str, Any]:
        ...
        # Configure RSMLT
        if (
            "rsmlt" in desired_config
            and desired_config["rsmlt"]
            and "instances" in desired_config["rsmlt"]
        ):
            ...
            for peer in peers:
                ...
                if peer_ip:
                    if not self._is_valid_ip(peer_ip):
                        self.module.fail_json(
                            msg="peer_ip_address is not a valid IP address: {}".format(
                                peer_ip
                            )
                        )
                ...
                if local_ip:
                    if not self._is_valid_ip(local_ip):
                        self.module.fail_json(
                            msg="local_ip_address is not a valid IP address: {}".format(
                                local_ip
                            )
                        )
                ...
                for instance in instances:
                    ...
                    if vlan_id is not None:
                        if (
                            ...
                            self.module.fail_json(
                                msg="vlan_id must be an integer between 1 and 4094"
                            )

                    hold_up_timer = instance.get("hold_up_timer")
                    if hold_up_timer is not None:
                        if (
                            not isinstance(hold_up_timer, int)
                            or (hold_up_timer < 0 or hold_up_timer > 3600)
                            and hold_up_timer != 9999
                        ):
                            self.module.fail_json(
                                msg="hold_up_timer must be an integer between 0 and 3600, or 9999 for infinity"
                            )

                    hold_down_timer = instance.get("hold_down_timer")
                    if hold_down_timer is not None:
                        if (
                            not isinstance(hold_down_timer, int)
                            or hold_down_timer < 0
                            or hold_down_timer > 3600
                        ):
                            self.module.fail_json(
                                msg="hold_down_timer must be an integer between 0 and 3600"
                            )
        ...
        try:
            ...
            if peers_response:
                for peer in peers_response:
                    ...
                    peer_ip_address = (
                        peer_ip_obj.get("address") if peer_ip_obj else None
                    )
                    ...
                    if include_ports:
                        try:
                            ports_response = self._send_request(
                                "GET", f"/v0/configuration/mlag/peers/{peer_id}/ports"
                            )
                            if ports_response:
                    ...
                    if state_peer:
                        ...
                        peer_data["local_ip_address"] = (
                            state_local_ip_obj.get("address")
                            if state_local_ip_obj
                            else None
                        )
                        ...
                        if include_state:
                            peer_data["state"] = {
                                "checkpointing_state": state_peer.get(
                                    "checkpointingState"
                                ),
            ...
            if include_rsmlt:
                try:
                    rsmlt_response = self._send_request(
                        "GET", "/v0/configuration/mlag/rsmlt"
                    )
                    if rsmlt_response:
                        rsmlt_list = (
                            rsmlt_response
                            if isinstance(rsmlt_response, list)
                            else [rsmlt_response]
                        )
                    else:
                        rsmlt_list = []
                    ...
                    if include_state:
                        try:
                            rsmlt_state_response = self._send_request(
                                "GET", "/v0/state/mlag/rsmlt"
                            )
                    ...
                    for vlan_config in rsmlt_list:
                        ...
                        for instance in rsmlt_instances:
                            ...
                            if include_state:
                                ...
                                if state_vlan:
                                    state_instances = state_vlan.get(
                                        "rsmltInstances", []
                                    )
                                    for state_instance in state_instances:
                                        instance_data["operational_state"] = (
                                            state_instance.get("operationalState")
                                        )
                                        break
    ...
    def _configure_peer(self, peer_config: Dict[str, Any]) -> None:
        ...
        self._send_request(
            "PATCH", f"/v0/configuration/mlag/peers/{peer_id}", peer_data
        )
    ...
    def _configure_peer_ports(
        self, peer_id: str, ports_config: List[Dict[str, Any]]
    ) -> None:
        ...
        for port_config in ports_config:
            ...
            port_data = {"portId": port_config["port_id"]}
            ports_data.append(port_data)

        self._send_request(
            "PUT", f"/v0/configuration/mlag/peers/{peer_id}/ports", ports_data
        )
        self.result["commands"].append(
            f"PUT /v0/configuration/mlag/peers/{peer_id}/ports"
        )
    ...
    def _configure_rsmlt_instance(self, rsmlt_config: Dict[str, Any]) -> None:
        ...
        self._send_request(
            "PATCH", f"/v0/configuration/mlag/rsmlt/vlan/{vlan_id}", instance_data
        )
        self.result["commands"].append(
            f"PATCH /v0/configuration/mlag/rsmlt/vlan/{vlan_id}"
        )
    ...
    def _delete_peer(self, peer_id: str) -> None:
        ...
        try:
            self._send_request(
                "PUT", f"/v0/configuration/mlag/peers/{api_peer_id}/ports", []
            )
            self.result["commands"].append(
                f"PUT /v0/configuration/mlag/peers/{api_peer_id}/ports (clear)"
            )
            deleted_ports = True
        except Exception:
    ...
    def _send_request(
        ...
        try:
            ...
            if "Method not found" in error_msg:
                self.module.fail_json(
                    msg=f"MLAG API endpoint {path} not supported on this device"
                )
                ...
                self.module.fail_json(
                    msg=f"Internal server error for {path}: {error_msg}"
                )
            else:
                self.module.fail_json(
                    msg=f"HTTP request failed for {path}: {error_msg}"
                )
                ...
                "peers": {
                    ...
                    "options": {
                        ...
                        "ports": {
                            "type": "list",
                            "elements": "dict",
                            "options": {"port_id": {"type": "str", "required": True}},
                        },
                    },
                },
                "rsmlt": {
